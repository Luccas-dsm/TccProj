...
namespace TccProj.Views.NFC
{
	...
	public partial class NfcView : ContentPage, INotifyPropertyChanged
    {
        ...
        protected async override void OnAppearing()
        {
            ...
            if (CrossNFC.IsSupported)
            {
             ...
             //       await Navigation.PopModalAsync();
                ...
                if (!NfcIsEnabled)
                    await ShowAlert("NFC está desativado");
        ...
        async void Current_OnMessageReceived(ITagInfo tagInfo)
        {
            if (tagInfo == null)
            {
                await ShowAlert("Nenhuma tag encontrada");
                return;
            }
        ...
        async Task Publish(NFCNdefTypeFormat? type = null)
        {
            ...
            try
            {
                ...
                {
                    if (!await DisplayAlert("Atenção", "Alterar a tag somente para leitura, não pode ser desfeito. Deseja continuar?", "Yes", "No"))
                    {
                        ChkReadOnly.IsChecked = false;
                        return;
                    }
